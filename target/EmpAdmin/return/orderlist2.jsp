...
<head>
    ...
    <script language="javascript">
        function confDeliver() {
            if (!confirm("Deliver this return product?")) {
...
<%--                用jstl标签进行数据的遍历--%>
                <c:forEach items="${orders}" var="order">
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="/return?opt=toEdit2&orderId=${order.orderId}&userName=${user.username}" class="btn btn-info btn-sm">Edit</a>
                            <a href="/return?opt=send&orderId=${order.orderId}&userName=${user.username}" class="btn btn-warning btn-sm" onClick="confDeliver()">Deliver</a>
                            <a href="/return?opt=cancel&orderId=${order.orderId}&userName=${user.username}" class="btn btn-danger btn-sm" onClick="confCancel()">Cancel</a>
                        </td>
                    </tr>
                    </c:if>
                </c:forEach>
            ...
            <table class="table table-striped table-bordered">
                ...
                <c:forEach items="${orders}" var="order">
                    ...
                    <tr>
                        ...
                        <td>${order.receiveTime}</td>
                        <td>
                            <a href="/return?opt=toDelete2&orderId=${order.orderId}&userName=${user.username}" class="btn btn-danger btn-sm">Delete</a>
                        </td>
                    </tr>
                    </c:if>
                </c:forEach>
